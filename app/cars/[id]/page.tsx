...
import { ArrowLeft, Heart, MessageCircle, Calendar, Fuel, Users, Settings, MapPin, Phone, Mail, Loader2 } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">{error || 'The car you\'re looking for doesn\'t exist.'}</p>
          <Link href="/rooms">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {car.images && car.images.length > 0 ? (
                ...
                <div className="w-full h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-500">Car image not available</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-4 gap-2">
                {car.images.map((image, index) => (
                  <button
                    ...
                    className={`relative h-20 rounded-lg overflow-hidden border-2 transition-colors ${selectedImage === index ? 'border-blue-600' : 'border-gray-200'
                      }`}
                  >
                    <img
                      src={image}
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xl text-gray-600">{car.year} {car.brand} {car.model}</p>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-foreground">
                      <span className="font-medium">Owner:</span> {car.ownershipHistory}
                    </span>
                  </div>
                  <div className="flex items-center">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="bg-secondary text-secondary-foreground">{car.condition}</Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="flex flex-wrap gap-3 sm:flex-nowrap">
              ...
              >
                <Heart
                  className={`h-4 w-4 ${favorites.includes(car.id) ? 'fill-current' : ''}`}
                />
              </Button>
              {user && user.id !== car.adminid?.id && (
                <Button onClick={() => setIsBookingDialogOpen(true)} className="bg-green-600 hover:bg-green-700">
            ...
            <Dialog open={isBookingDialogOpen} onOpenChange={setIsBookingDialogOpen}>
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Fill in your details to book a test drive or inquire about the car.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  ...
                  {bookingError && <p className="text-red-500 text-sm text-center">{bookingError}</p>}
                  {bookingSuccess && <p className="text-green-500 text-sm text-center">Booking request sent!</p>}
                </div>
                <DialogFooter>
                  <Button
                    type="submit"
            ...
            <Card className="bg-card text-card-foreground">
              ...
              <CardContent>
                <div className="space-y-3">
                  <div>
                    <p className="font-medium text-foreground">{car.roomId.name}</p>
                    <p className="text-sm text-muted-foreground">{car.roomId.location}</p>
                  </div>

                  <div>
                    <p className="text-sm text-muted-foreground">Contact Person: {car.adminid?.username || 'Unknown'}</p>
                  </div>

                  {car.roomId.contactInfo && (
                    <div className="space-y-2">
                      {car.roomId.contactInfo.phone && (
                        <div className="flex items-center text-sm text-foreground">
                          <Phone className="h-4 w-4 mr-2 text-muted-foreground" />
                          {car.roomId.contactInfo.phone}
                        </div>
                      )}
                      {car.roomId.contactInfo.email && (
                        <div className="flex items-center text-sm text-foreground">
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {car.specifications && (
            <Card className="bg-card text-card-foreground">
              <CardHeader>
                <CardTitle className="text-card-foreground">Detailed Specifications</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  ...
                  {car.specifications.torque && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-foreground">{car.specifications.torque}</span>
                    </div>
                  )}
                  {car.specifications.acceleration && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">0-60 mph:</span>
                      <span className="font-medium text-foreground">{car.specifications.acceleration}</span>
                    </div>
                  )}
                  {car.specifications.topSpeed && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Top Speed:</span>
                      <span className="font-medium text-foreground">{car.specifications.topSpeed}</span>
                    </div>
                  )}
                </div>
